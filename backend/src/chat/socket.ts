...
export class ChatSocket {
  ...
  private setupEventHandlers() {
    this.io.on('connection', (socket: AuthenticatedSocket) => {
      ...
      socket.on('message:send', async (data: { conversationId: number; content: string; messageType?: string; mediaUrls?: string[] }) => {
        try {
          const { conversationId, content, messageType = 'text', mediaUrls = [] } = data;
          ...
          if (!member) {
            socket.emit('message:error', { error: 'You are not a member of this conversation' });
            return;
          }
          ...
          this.io.to(`conversation:${conversationId}`).emit('message:new', formattedMessage);
      ...
      socket.on('message:read', async (data: { conversationId: number }) => {
        try {
          ...
          this.io.to(`conversation:${data.conversationId}`).emit('messages:read', {
            conversationId: data.conversationId,
            readerId: userId,
            count: result.count,
          });
    ...
    socket.on('message:delete', async (data: { conversationId: number; messageId: number }) => {
    try {
        const prisma = getPrisma();
        ...
        this.io.to(`conversation:${data.conversationId}`).emit('message:deleted', {
        conversationId: data.conversationId,
      ...
      socket.on('user:check_online', (data: { userId: number }, callback: (response: any) => void) => {
  ...
  private async joinUserConversations(socket: Socket, userId: number) {
    try {
      ...
      for (const membership of memberships) {
        ...
        console.log(`User ${userId} auto-joined conversation ${membership.conversationId}`);
      }
    } catch (error) {
